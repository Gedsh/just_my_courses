...
class LocalRepositoryImplementation : LocalRepository {
    ...
    override fun getNowPlayingFilms(): LiveData<List<Film>> {
        return filmDao.get().getFilmsForSection(Film.Section.NOW_PLAYING.value)
    }

    override fun getUpcomingFilms(): LiveData<List<Film>> {
        return filmDao.get().getFilmsForSection(Film.Section.UPCOMING.value)
    }

    override fun getTopRatedFilms(): LiveData<List<Film>> {
        return filmDao.get().getFilmsForSection(Film.Section.TOP_RATED.value)
    }

    override fun getPopularFilms(): LiveData<List<Film>> {
        return filmDao.get().getFilmsForSection(Film.Section.POPULAR.value)
    }

    override fun getLikedFilms(): LiveData<List<Film>> {
        return filmDao.get().getLikedFilms()
    }

    override fun addFilm(film: Film) {
        filmDao.get().insert(film)
    }

    override fun addFilms(films: List<Film>) {
        filmDao.get().insertFilms(films)
    }
    ...
    override fun updateFilm(film: Film) {
        filmDao.get().update(film)
    }

    override fun getFilmDetailsById(id: Int): LiveData<List<FilmDetails>> {
        return filmDetailsDao.get().getFilmDetailsById(id)
    }
